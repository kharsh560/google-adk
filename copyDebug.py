...
def load_image_as_binary(filename: str = "OfficialMarks.jpeg") -> tuple[bytes, str]:
    """Load an image file from the same directory and return its binary data and MIME type.
    
    Args:
        filename (str): Name of the image file to load (defaults to "OfficialMarks.jpeg")
...
# await save_upload_as_artifact(context, "OfficialMarks.jpeg", bytesFile, mimeType)
...
async def init_session(app_name:str,user_id:str,session_id:str) -> InMemorySessionService:
    ...
    artifact_id = await save_upload_backend(
        session_id=SESSION_ID,
        original_filename="OfficialMarks.jpeg",
...
async def run_conversation():
    await call_agent_async("What is the SID present in this image and what is the name of the student whose scorecard it this?",
                                       runner=runner,
                                       user_id=USER_ID,
                                       session_id=SESSION_ID)
