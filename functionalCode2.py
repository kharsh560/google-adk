...
# async def save_upload_as_artifact(
#     context: CallbackContext,
#     filename: str,
#     file_bytes: bytes,
#     mime_type: str
# ) -> str:
#     """Save uploaded file bytes to ArtifactService and return its artifact ID."""
#     # Create a Part with the uploaded bytes
#     artifact_part = Part(
#         inline_data={'mime_type': mime_type, 'data': file_bytes},
#         text=f"File uploaded: {filename}"
#     )

#     # Save it to the session’s artifact store
#     version = await context.save_artifact(filename=filename, artifact=artifact_part)

#     # Return the artifact name used (filename)
#     return filename
...
# async def before_model_include_artifact(
#     callback_context: CallbackContext,
#     llm_request: LlmRequest
# ):
#     """Add artifact reference text to the LLM request."""
#     # Suppose your agent instruction uses {artifact.<filename>}
#     # Look up artifact list for this session
#     artifact_list = await callback_context.list_artifacts()
    
#     print(f"Listing Artifacts: '{artifact_list}'")

#     # Insert artifact names into the instruction
#     if artifact_list:
#         artifact_refs = ", ".join(artifact_list)
#         for content in llm_request.contents:
#             # Prepend artifact info in the prompt
#             content.parts.insert(
#                 0,
#                 Part(text=f"Session has uploaded artifacts: {artifact_refs}\n")
#             )
#             print(f"Listing content: '{content}'")
#     return None

# Move photo_agent creation after session initialization
...
async def run_conversation():
    await call_agent_async("What is the name and enrollment number present in this scorecard image and what is the total score of the subject with papercode '443231'?",
                                       runner=runner,
                                       user_id=USER_ID,
                                       session_id=SESSION_ID)
...
if __name__ == "__main__":
    try:
        ...
        print(f"An error occurred: {e}")
            ...
            instruction=(
                """
                You are analyzing an official academic marksheet image from IP University.

                Your role is to:
                1. Understand the structural layout of the marksheet
                2. Answer user questions strictly based on visible data
                3. Calculate GPA/CGPA ONLY when explicitly asked, using the official IP University rules defined below

                ────────────────────
                MARKSHEET STRUCTURE
                ────────────────────
                1. The top-left section contains student metadata (roll number, name, SID, scheme ID).
                2. Each subject is identified by a unique paper code shown at the top of a column.
                3. Under each paper code:
                - The first numeric value represents theory marks.
                - The second numeric value represents practical marks.
                - A dash ("-") means that component does not exist.
                4. The bottom-most value aligned under each paper code represents:
                - Total marks
                - Letter grade (e.g., O, A+, A)
                5. All interpretations must be based strictly on visual alignment and position.

                ────────────────────
                IP UNIVERSITY GRADING RULES
                ────────────────────
                Use the following grade-point mapping exactly:
                ...
                Grades must never be reinterpreted or modified.

                ────────────────────
                CGPA / GPA CALCULATION RULES
                ────────────────────
                When asked to calculate GPA or CGPA, use the IP University formula:

                CGPA = Σ(C × G) / Σ(C)

                Where:
                - C = credit value of the subject
                - G = grade point corresponding to the subject’s grade

                Calculation rules:
                1. Use only subjects that are visible in the image.
                2. If credit values are NOT visible in the image:
                - Assume equal weight for all subjects.
                - Clearly state this assumption in the answer.
                3. Do NOT include failed or absent subjects unless explicitly asked.
                4. Round the final GPA/CGPA to **2 decimal places**.
                5. Do NOT calculate GPA unless the user explicitly asks for it.

                ────────────────────
                ANSWERING RULES
                ────────────────────
                - Do not guess or infer missing data.
                - If required data is not visible, say:
                "The required information is not visible in the image."
                - Be concise, factual, and calculation-transparent.
                - Do not output structured data unless explicitly requested.

                ────────────────────
                EXAMPLES OF ALLOWED QUESTIONS
                ────────────────────
                - What are the marks for paper code 116204?
                - What grade did I get in 116210?
                - Calculate my GPA based on this marksheet.
                - What grade point corresponds to paper code 116252?
                """
            ),